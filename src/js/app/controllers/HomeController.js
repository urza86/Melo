...
myApp.controller('HomeController', ['$scope', '$http','helperService','$location','$log',  function ( $scope, $http, helperService, $location, $log) {
        $scope.greeting = 'Hola!';
        ...
        $scope.myResults = helperService.inmueblesDB;

        $log.info($scope.myResults);
    ...
    $scope.results = helperService.inmueblesDB.dataset;
      /*  $http.get('/inmuebles').then(function (data) {
        ...
        $scope.restrictCharacters = function (paragraph){
            ...
            return paragraph.substring(0, 75) + "...";
        ...
        $scope.busquedaAvanzada = function () {
           ...
           //
            ...
            $('#loader').css("margin-top", "20%");
            $('#loader').show();
            setTimeout(window.location.assign("/src/views/searchPage.html?" + parameters), 2000);
